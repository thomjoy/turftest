  ...
  JTSTestFactory.generate = function(doc, title) {
    ...
    if (precisionModelInfo.length === 1) {
      ...
      if (type !== 'FLOATING') {
        ...
        geometryFactory = new jsts.geom.GeometryFactory(new jsts.geom.PrecisionModel(scale));
      }
    }
    ...
    var generateSpec = function(a, b, opname, arg2, arg3, expected) {
      it('Executing ' + opname + ' on test geometry', function() {
        ...
        if (opname === 'relate' || opname === 'contains' || opname === 'intersects' || opname === 'equalsExact' || opname === 'equalsNorm' || opname === 'isSimple') {
          var expectedBool = expected === 'true';
          if (expectedBool !== result) {
            throw new Error('Result: ' + result + ' Expected: ' + expected + inputs);
          } else {
          ...
          if (result !== expectedDistance) {
            throw new Error('Result: ' + result + ' Expected: ' + parseFloat(expectedDistance) + inputs);
          } else {
            expect(true).toBeTruthy();
          }
          ...
          if (!matcher.isBufferResultMatch(result, expectedGeometry, parseFloat(arg2))) {
            throw new Error('Result: ' + result + ' Expected: ' + expectedGeometry + inputs);
          } else {
            expect(true).toBeTruthy();
          }
        } else {
